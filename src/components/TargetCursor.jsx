...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    if (hideDefaultCursor && !sectionEl) {
    ...
    return () => {
      ...
      if (sectionEl && hideDefaultCursor) {
  ...
  useEffect(() => {
    if (isMobile || !sectionSelector) return;
    const el = document.querySelector(sectionSelector);
    if (!el) return;
    const enter = () => setIsInSection(true);
    const leave = () => setIsInSection(false);
    if (el.matches(':hover')) {
      setIsInSection(true);
    }
  ...
  const visible = !sectionSelector || isInSection;

  return (
    <div ref={cursorRef} className={`target-cursor-wrapper${visible ? ' in-section' : ''}`}>
